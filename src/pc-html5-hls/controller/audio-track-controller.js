...
class AudioTrackController extends EventHandler {
  ...
  constructor(AVPLAYER) {
    super(AVPLAYER, Event.MANIFEST_LOADING,
  ...
  onManifestLoaded(data) {
    ...
    this.tracks = tracks;
    this.AVPLAYER.trigger(Event.AUDIO_TRACKS_UPDATED, {audioTracks : tracks});
  ...
  updateTrack(newId) {
    ...
    if (newId >= 0 && newId < this.tracks.length) {
      ...
      if (url && (details === undefined || details.live === true)) {
        // track not retrieved yet, or live playlist we need to (re)load it
        logger.log(`(re)loading playlist for audioTrack ${newId}`);
        this.AVPLAYER.trigger(Event.AUDIO_TRACK_LOADING, {url: url, id: newId});
      }
    }
  }
}
