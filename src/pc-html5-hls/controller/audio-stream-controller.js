...
class AudioStreamController extends EventHandler {
  ...
  constructor(AVPLAYER) {
    ...
    this.config = this.AVPLAYER.config;
  ...
  doTick() {
    var pos, track, trackDetails, hls = this.AVPLAYER, config = this.AVPLAYER.config;
      ...
      case State.IDLE:
        ...
        if (bufferLen < maxBufLen && trackId < tracks.length) {
          ...
          if (!audioSwitch && !trackDetails.live && fragPrevious && fragPrevious.sn === trackDetails.endSN) {
              ...
              this.AVPLAYER.trigger(Event.BUFFER_EOS,{ type : 'audio'});
          ...
          if(frag) {
            ...
            if (frag.decryptdata && (frag.decryptdata.uri != null) && (frag.decryptdata.key == null)) {
              ...
              this.AVPLAYER.trigger(Event.KEY_LOADING, {frag: frag});
            } else {
              ...
              if (frag.loadCounter) {
                ...
                if (frag.loadCounter > maxThreshold && (Math.abs(this.fragLoadIdx - frag.loadIdx) < maxThreshold)) {
                  this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.MEDIA_ERROR, details: ErrorDetails.FRAG_LOOP_LOADING_ERROR, fatal: false, frag: frag});
                  return;
                }
              } else {
                frag.loadCounter = 1;
              ...
              this.AVPLAYER.trigger(Event.FRAG_LOADING, {frag: frag});
              this.state = State.FRAG_LOADING;
            }
          }
        }
  ...
  onFragLoaded(data) {
    ...
    if (this.state === State.FRAG_LOADING &&
      ...
      if (sn === 'initSegment') {
        ...
        this.AVPLAYER.trigger(Event.FRAG_BUFFERED, {stats: stats, frag: fragCurrent, id : 'audio'});
        this.tick();
      } else {
        ...
        this.appended = false;
        if(!this.demuxer) {
          this.demuxer = new Demuxer(this.AVPLAYER,'audio');
        }
      ...
      if(track) {
        ...
        if (initSegment) {
          ...
          if (this.audioSwitch) {
            ...
            this.pendingBuffering = true;
            this.AVPLAYER.trigger(Event.BUFFER_APPENDING, appendObj);
          }
        }
        //trigger handler right now
        this.tick();
      ...
      if(audioSwitch && media) {
        if (media.readyState) {
          ...
          if (currentTime >= data.startPTS) {
            ...
            this.AVPLAYER.trigger(Event.BUFFER_FLUSHING, {startOffset: 0 , endOffset: Number.POSITIVE_INFINITY, type : 'audio'});
            appendOnBufferFlush = true;
            //Lets announce that the initial audio track switch flush occur
            this.audioSwitch = false;
            this.AVPLAYER.trigger(Event.AUDIO_TRACK_SWITCHED, {id : trackId});
          }
        } else {
          //Lets announce that the initial audio track switch flush occur
          this.audioSwitch=false;
  ...
  _checkAppendedParsed() {
    ...
    if (this.state === State.PARSED && (!this.appended || !this.pendingBuffering))   {
      let frag = this.fragCurrent, stats = this.stats, hls = this.AVPLAYER;
      if (frag) {
        this.fragPrevious = frag;
        stats.tbuffered = performance.now();
        this.AVPLAYER.trigger(Event.FRAG_BUFFERED, {stats: stats, frag: frag, id : 'audio'});
        let media = this.mediaBuffer ? this.mediaBuffer : this.media;
        logger.log(`audio buffered : ${TimeRanges.toString(media.buffered)}`);
        if (this.audioSwitch && this.appended) {
          this.audioSwitch = false;
          this.AVPLAYER.trigger(Event.AUDIO_TRACK_SWITCHED, {id : this.trackId});
        }
        this.state = State.IDLE;
      }
      this.tick();
