...
class StreamController extends EventHandler {
  ...
  startLoad(startPosition) {
    if (this.levels) {
      let lastCurrentTime = this.lastCurrentTime, hls = this.AVPLAYER;
      this.stopLoad();
      if (!this.timer) {
  ...
  _doTickIdle() {
    ...
    if (!levelDetails.live && fragPrevious && fragPrevious.sn === levelDetails.endSN) {
        ...
        this.AVPLAYER.trigger(Event.BUFFER_EOS,data);
  ...
  _ensureFragmentAtLivePoint(levelDetails, bufferEnd, start, end, fragPrevious, fragments, fragLen) {
    const config = this.AVPLAYER.config, media = this.media;
  ...
  _findFragment(start, fragPrevious, fragLen, fragments, bufferEnd, end, levelDetails) {
    const config = this.AVPLAYER.config;
    let frag;
    let foundFrag;
  ...
  set state(nextState) {
    if (this.state !== nextState) {
      ...
      this.AVPLAYER.trigger(Event.STREAM_STATE_TRANSITION, {previousState, nextState});
    }
  }
      ...
      */
      ...
      if (fragPlayingCurrent) {
        ...
        if (fragPlaying !== this.fragPlaying) {
          this.AVPLAYER.trigger(Event.FRAG_CHANGED, {frag: fragPlaying});
          const fragPlayingLevel = fragPlaying.level;
          if (!this.fragPlaying || this.fragPlaying.level !== fragPlayingLevel) {
            this.AVPLAYER.trigger(Event.LEVEL_SWITCHED, {level: fragPlayingLevel});
          }
  ...
  */
    ...
    */
    ...
    if (media && media.readyState) {
      ...
      if (!media.paused) {
        ...
        var nextLevelId = this.AVPLAYER.nextLoadLevel,nextLevel = this.levels[nextLevelId], fragLastKbps = this.fragLastKbps;
  ...
  flushMainBuffer(startOffset,endOffset) {
    ...
    this.AVPLAYER.trigger(Event.BUFFER_FLUSHING, flushScope);
  }
  ...
  onManifestLoading() {
    ...
    this.AVPLAYER.trigger(Event.BUFFER_RESET);
    this._bufferedFrags = [];
    this.stalled = false;
    this.startPosition = this.lastCurrentTime = 0;
  }
  ...
  onLevelLoaded(data) {
    ...
    this.AVPLAYER.trigger(Event.LEVEL_UPDATED, { details: newDetails, level: newLevelId });
  ...
  onFragLoaded(data) {
    ...
    if (this.state === State.FRAG_LOADING &&
      ...
      if (fragLoaded.bitrateTest === true && this.AVPLAYER.nextLoadLevel) {
        ...
        this.AVPLAYER.trigger(Event.FRAG_BUFFERED, {stats: stats, frag: fragCurrent, id : 'main'});
        this.tick();
        ...
        if (!demuxer) {
          demuxer = this.demuxer = new Demuxer(this.AVPLAYER,'main');
        }
  ...
  onFragParsingInitSegment(data) {
    ...
    if (fragCurrent &&
      ...
      this.AVPLAYER.trigger(Event.BUFFER_CODECS,tracks);
      ...
      for (trackName in tracks) {
        ...
        if (initSegment) {
          ...
          this.pendingBuffering = true;
          this.AVPLAYER.trigger(Event.BUFFER_APPENDING, {type: trackName, data: initSegment, parent : 'main', content : 'initSegment'});
        }
      }
      //trigger handler right now
      this.tick();
  ...
  _checkAppendedParsed() {
    ...
    if (this.state === State.PARSED && (!this.appended || !this.pendingBuffering)) {
      ...
      if (frag) {
        ...
        this.AVPLAYER.trigger(Event.FRAG_BUFFERED, {stats: stats, frag: frag, id : 'main'});
        this.state = State.IDLE;
      }
      this.tick();
    }
