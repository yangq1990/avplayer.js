...
class SubtitleTrackController extends EventHandler {
  ...
  onManifestLoaded(data) {
    ...
    this.tracks = tracks;
    this.trackId = -1;
    this.AVPLAYER.trigger(Event.SUBTITLE_TRACKS_UPDATED, {subtitleTracks : tracks});
  ...
  onTick() {
    ...
    if (details === undefined || details.live === true) {
      ...
      this.AVPLAYER.trigger(Event.SUBTITLE_TRACK_LOADING, {url: subtitleTrack.url, id: trackId});
    }
  }
 ...
 setSubtitleTrackInternal(newId) {
    ...
    if (newId >= 0 && newId < this.tracks.length) {
      ...
      this.AVPLAYER.trigger(Event.SUBTITLE_TRACK_SWITCH, {id: newId});
       // check if we need to load playlist for this subtitle Track
      let details = subtitleTrack.details;
      if (details === undefined || details.live === true) {
        // track not retrieved yet, or live playlist we need to (re)load it
        logger.log(`(re)loading playlist for subtitle track ${newId}`);
        this.AVPLAYER.trigger(Event.SUBTITLE_TRACK_LOADING, {url: subtitleTrack.url, id: newId});
      }
    }
  }
}
