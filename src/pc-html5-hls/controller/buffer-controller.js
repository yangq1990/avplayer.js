...
class BufferController extends EventHandler {
  ...
  onMediaAttaching(data) {
    ...
    if (media) {
      ...
      AVLog.print(`media.src->${media.src}`);
    }
  }
  ...
  onMediaDetaching() {
    ...
    this.AVPLAYER.trigger(Event.MEDIA_DETACHED);
  }

  onMediaSourceOpen() {
    this.AVPLAYER.trigger(Event.MEDIA_ATTACHED, { media : this.media });
  ...
  onSBUpdateEnd() {
    ...
    this.AVPLAYER.trigger(Event.BUFFER_APPENDED, { parent : parent, pending : pending });
  ...
  onSBUpdateError(event) {
    ...
    this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.MEDIA_ERROR, details: ErrorDetails.BUFFER_APPENDING_ERROR, fatal: false});
    ...
    for (let trackName in tracks) {
      if(!sourceBuffer[trackName]) {
        ...
        try {
          ...
          this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.MEDIA_ERROR, details: ErrorDetails.BUFFER_ADD_CODEC_ERROR, fatal: false, err: err, mimeType : mimeType});
        }
      }
    }
    this.AVPLAYER.trigger(Event.BUFFER_CREATED, { tracks : tracks } );
  }
 ...
 checkEos() {
  ...
  doFlush() {
    ...
    if (this.flushRange.length === 0) {
      ...
      this.AVPLAYER.trigger(Event.BUFFER_FLUSHED);
    }
  }

  doAppending() {
    var hls = this.AVPLAYER, sourceBuffer = this.sourceBuffer, segments = this.segments;
          ...
          if(err.code !== 22) {
            ...
            */
            if (this.appendError > AVPLAYER.config.appendErrorMaxRetry) {
              logger.log(`fail ${AVPLAYER.config.appendErrorMaxRetry} times to append segment in sourceBuffer`);
              segments = [];
              event.fatal = true;
              this.AVPLAYER.trigger(Event.ERROR, event);
              return;
            } else {
              event.fatal = false;
              this.AVPLAYER.trigger(Event.ERROR, event);
