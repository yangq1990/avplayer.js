...
class AbrController extends EventHandler {
  ...
  _abandonRulesCheck() {
    ...
    */
    let hls = this.AVPLAYER, v = hls.media,frag = this.fragCurrent, loader = frag.loader, minAutoLevel = hls.minAutoLevel;
  ...
  onFragLoaded(data) {
    ...
    if (frag.type === 'main' && !isNaN(frag.sn)) {
      ...
      if (this.AVPLAYER.config.abrMaxWithRealBitrate) {
        const level = this.AVPLAYER.levels[frag.level];
 ...
 clearTimer() {
  ...
  get _nextABRAutoLevel() {
    var hls = this.AVPLAYER, maxAutoLevel = hls.maxAutoLevel, levels = hls.levels, config = hls.config, minAutoLevel = hls.minAutoLevel;
    const v = hls.media,
