...
class KeyLoader extends EventHandler {
  ...
  constructor(AVPLAYER) {
    super(AVPLAYER, Event.KEY_LOADING);
    this.loaders = {};
  ...
  onKeyLoading(data) {
    let frag = data.frag,
      ...
      if (uri !== this.decrypturl || this.decryptkey === null) {
        ...
        this.AVPLAYER.trigger(Event.KEY_LOADED, {frag: frag});
      }
  }
  ...
  loaderror(response, context) {
    ...
    this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, details: ErrorDetails.KEY_LOAD_ERROR, fatal: false, frag: frag, response: response});
  }
  ...
  loadtimeout(stats, context) {
    ...
    this.loaders[context.type] = undefined;
    this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, details: ErrorDetails.KEY_LOAD_TIMEOUT, fatal: false, frag: frag});
  }
}
