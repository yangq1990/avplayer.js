...
class PlaylistLoader extends EventHandler {
  ...
  load(url, context) {
    ...
    let config = this.AVPLAYER.config,
        retry,
        timeout,
        retryDelay,
        maxRetryDelay;
  ...
  parseLevelPlaylist(string, baseurl, id, type) {
    ...
    AVLog.print(`视频总时长 ${totalduration} s | ts平均时长 ${level.averagetargetduration} s`);
  ...
  loadsuccess(response, stats, context) {
    ...
    if (string.indexOf('#EXTM3U') === 0) {
      if (string.indexOf('#EXTINF:') > 0) {
        ...
        AVLog.print(`解析m3u8文件耗时 ${toFixed(stats.tparsed - stats.tload)} ms`);

        if (type === 'manifest') {
        // first request, stream manifest (no master playlist), fire manifest loaded event with level details
          this.AVPLAYER.trigger(Event.MANIFEST_LOADED, {levels: [{url: url, details : levelDetails}], audioTracks : [], url: url, stats: stats});
        }
        
        if (levelDetails.targetduration) {
          if (isLevel) {
            this.AVPLAYER.trigger(Event.LEVEL_LOADED, {details: levelDetails, level: level || 0, id: id || 0, stats: stats});
          } else {
            if (type === 'audioTrack') {
              this.AVPLAYER.trigger(Event.AUDIO_TRACK_LOADED, {details: levelDetails, id: id, stats: stats});
            }
            else if (type === 'subtitleTrack') {
              this.AVPLAYER.trigger(Event.SUBTITLE_TRACK_LOADED, {details: levelDetails, id: id, stats: stats});
            }
          }
        } else {
          this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, details: ErrorDetails.MANIFEST_PARSING_ERROR, fatal: true, url: url, reason: 'invalid targetduration'});
        }
      } else {
        ...
        if (levels.length) {
          ...
          this.AVPLAYER.trigger(Event.MANIFEST_LOADED, {levels, audioTracks, subtitles, url, stats});
        } else {
          this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, details: ErrorDetails.MANIFEST_PARSING_ERROR, fatal: true, url: url, reason: 'no level found in manifest'});
        }
      }
    } else {
      this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, details: ErrorDetails.MANIFEST_PARSING_ERROR, fatal: true, url: url, reason: 'no EXTM3U delimiter'});
    }
  }
  ...
  loaderror(response, context) {
    ...
    this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, details: details, fatal: fatal, url: loader.url, loader: loader, response: response, context : context});
  }
  ...
  loadtimeout(stats, context) {
    ...
    this.AVPLAYER.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, details: details, fatal: fatal, url: loader.url, loader: loader, context : context});
  }
}
